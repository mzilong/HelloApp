...
function Chess(){
	...
	function man(width,height,id,colorBak){
		var color=colorBak==null?(order++%2==0?"000":"FFF"):colorBak;
		var r="border-radius:"+width/2+"px;";
		var obj=id==null?event.srcElement:_$(id);
		obj.innerHTML="<div id=\"man_"+color+"_"+order+"\" style=\"display:block;-webkit-"+r+"-moz-"+r+""+r+"-moz-box-shadow:inset 0 -10px 40px rgba(0,0,0,1);background:-webkit-gradient(radial, 50 40, 30, center center, 80, from(#"+color+"), to(#"+color+"));width:"+width+"px;height:"+height+"px;\"></div>";
	}
	/*绘制多个棋子*/
	...
	/*判断是否有棋子*/function isMan(row,col){var obj=_$(nameBak+"_"+row+"_"+col,1);if(obj==null||obj.indexOf("man_")==-1)return null;else if(obj.indexOf("000")!=-1)return 0;else if(obj.indexOf("FFF")!=-1)return 1;}
